...
def _guess_value(arg_name: str) -> str:
    """Heuristically choose a plausible sample value for a function argument."""
...
def _build_test_body(func_name: str, args: list[str], edge_cases: list[str]) -> str:
    """Assemble the body of a pytest function for the given target callable."""
...
def _build_test_file(module_path: str, signatures: list[dict], edge_cases: list[str]) -> str:
    """Render a pytest file that imports the target module and exercises its callables."""
    lines = [
        "import importlib",
        "import pytest",
...
async def _llm_test_suggestions(code: str, analysis: dict) -> list[dict]:
    """Call the configured LLM for additional targeted tests when enabled."""
...
def _append_llm_tests(base_code: str, llm_tests: list[dict]) -> str:
    """Append model-suggested tests to the generated pytest file."""
    if not llm_tests:
        return base_code
